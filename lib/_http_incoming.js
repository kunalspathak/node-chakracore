...
function getHttpVersion(versionMajor, versionMinor) {
  if (versionMajor == 0 && versionMinor == 9) {
    return '0.9';
  } else if (versionMajor == 1 && versionMinor == 0) {
    return '1.0';
...
function _addHeaderLines(headers, n) {
  if (headers && headers.length) {
    ...
    for (var i = 0, headerIndex = 0; i < n; i += 4, headerIndex += 2) {
      var rawHeader = headers[i];
      var value = headers[i + 1];
      var lowerCaseHeader = headers[i + 2];
      var flag = headers[i + 3];
      this._addHeaderLine(rawHeader, value, dest, lowerCaseHeader, flag);
      headers[headerIndex] = rawHeader;
      headers[headerIndex + 1] = value;
    }
    headers.length = headerIndex;
  }
}
...
function _addHeaderLine(field, value, dest, lowercaseField, flag) {
  if (arguments.length > 3) {
    field = lowercaseField;
  } else {
    field = matchKnownFields(field);
    flag = field.charCodeAt(0);
    field = flag != 0 ? field : field.slice(1);
  }
  switch (flag) {
    case 0:
